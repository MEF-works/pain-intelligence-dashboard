...
import { capturePainSignal, hashContent } from './run-ingest';
...
/**
 * Serper-backed google_dork ingest. Uses the same `DATABASE_PATH` / DB as the Next app
 * (call from `GET /api/cron/ingest-dorks` in Docker so rows appear in the dashboard).
 *
 * @returns count of newly stored rows this run
 */
export async function runDorkIngest(): Promise<number> {
  const rawDb = process.env.DATABASE_PATH?.trim();
  const resolvedDb = rawDb ? path.resolve(rawDb) : path.join(process.cwd(), 'data', 'pain.db');
